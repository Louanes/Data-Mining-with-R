...
load("/Users/jihan/Downloads/entropy_data.RData")
head(entropy.data)
...
getInfoGain <- function(split_var, class_var){
  ...
  for(i in 1:nrow(attr.class.freq)){
    ...
    # *** ATTENTION !!! *** #
    child.node.freq <-  attr.class.freq[i,] #"FIX ME"
    ...
    # *** ATTENTION !!! *** #
    entropy_split <- entropy_split + ((num_obs_child.node / num_obs_parent.node) * child.node.entropy) #"FIX ME"
  ...
  for(i in 1:number_of_columns){
    ...
    if(class(data.X[,i]) == "numeric"){
      ...
      for(j in 1:(length(x_sort_unique)-1)){
        ...
        # *** ATTENTION !!! *** #
        cutoff_point <- (x_sort_unique[j] + x_sort_unique[j + 1]) / 2 #"FIX ME"
        ...
        # *** ATTENTION !!! *** #
        info_gain_for_this_split <- getInfoGain(lessthan_cutoff, data.Y) #"FIX ME"
...
load("/Users/jihan/Downloads/Perceptron_data.RData")
...
my.perceptron <- function(data.X, data.Y, max.iter = 500){
  ...
  while(TRUE){
    ...
    # *** ATTENTION !!! *** #
    hypothesis <- as.matrix((X.new)) %*% as.matrix(t(w)) #"FIX ME"
    ...
    if(all(label.new == data.Y)){ 
      ...
      # *** ATTENTION !!! *** #
      w <- (w + data.Y[rand.sel.index] * X.new[rand.sel.index,]) #"FIX ME"
...
# *** ATTENTION !!! *** #
k.fold.data.idx <- createFolds(heart.disease.db$class, k = num.folds) #"FIX ME"
...
for(i in 1:length(cost.parameter)){
  ...
  for(j in 1:num.folds){
    ...
    # *** ATTENTION !!! *** #
    svm.model <- svm(formula = class ~ ., data = train.set, kernel = "radial", cost = cost.parameter[i], probability = TRUE) #"FIX ME"
...
# *** ATTENTION !!! *** #
svm.pred <-  predict(svm.model, newdata = test.set, probability = TRUE) #"FIX ME"

class.probs <- attr(svm.pred, "probabilities")
pred2 <- prediction(class.probs[,2], test.set$class)

# *** ATTENTION !!! *** #
roc.curve <- performance(pred2, "tpr", "fpr") #"FIX ME"
